...
public class MinimizingMakeChangeFitnessFunction
  ...
  public static final int extinction_factor = 20;
  ...
  public double evaluate(IChromosome a_subject) {
    ...
    //Step3 Neric: we want to always have at least one coin. 

     fitness = extinctionPenality(fitness,a_subject);
  ...
  protected double changeDifferenceBonus(double a_maxFitness,
    ...
    if (a_changeDifference == 0) {
      ...
      if (a_changeDifference * a_changeDifference >= a_maxFitness / 2) {
        ...
        return a_maxFitness / 2 - distance_factor*a_changeDifference * a_changeDifference;
  ...
  protected double computeCoinNumberPenalty(double a_maxFitness, int a_coins) {
    if (a_coins == 1) {
      ...
      //return (Math.min(a_maxFitness, a_coins * a_coins));
    	return (Math.min(a_maxFitness, a_coins));
    }
  }
  /**
   * Calculates the penalty to apply to the fitness value if one of the coin 
   * is zero. we want there is at least one coin at each catergory
   *
   * @return penalty for the fitness value if one of the coin type is missing
   *
   * @author Neric Lau
   * @since 1.0
   */  
  protected double extinctionPenality(double a_maxFitness,IChromosome a_potentialSolution){
	  //quarters
	  if ( this.getNumberOfCoinsAtGene(a_potentialSolution, 0) ==0 ){
		  	return a_maxFitness/extinction_factor;
	  }
	  else if ( this.getNumberOfCoinsAtGene(a_potentialSolution, 1)==0 ) {
		    return a_maxFitness/extinction_factor;
	  }
	  else if ( this.getNumberOfCoinsAtGene(a_potentialSolution, 2)==0 ) {
		    return a_maxFitness/extinction_factor;
	  }
	  else if ( this.getNumberOfCoinsAtGene(a_potentialSolution, 3)==0 ) {
		    return a_maxFitness/extinction_factor;
	  }
	  return a_maxFitness;
  }
